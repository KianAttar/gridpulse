...
const ForecastChart = dynamic(
  () => import("./ForecastChart").then((m) => m.ForecastChart),
  {
    ssr: false,
    loading: () => (
      <div className="rounded-lg border border-border bg-card min-h-64 animate-pulse" />
    ),
  },
);
            ...
            <div className="flex items-start justify-between border-b border-border p-5">
              <div className="flex flex-col gap-1">
                ...
                <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                  <span
                    className={`h-1.5 w-1.5 rounded-full ${statusConfig[node.status].dot}`}
                  />
                  <span className={statusConfig[node.status].text}>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="h-1.5 w-full rounded-full bg-muted overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${Math.min((node.powerDraw / MAX_POWER_W) * 100, 100)}%`,
                        }}
                      />
                    </div>
                    <p className="text-xs text-muted-foreground">
                      {Math.round((node.powerDraw / MAX_POWER_W) * 100)}% of
                      reference capacity
                    </p>
                  </div>
                )}
